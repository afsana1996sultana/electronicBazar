...
if (!function_exists('get_categories')) {
    function get_categories()
    {
        ...
        $cats = Category::where('id', '>', 1)->where('status', 1)->latest()->get();
        foreach($cats as $cat){
            $has_sub_sub = 0;
            if($cat->parent_id==0){
...
if (!function_exists('header_menu_categories')) {
    function header_menu_categories()
    {
        //$categories = Category::where('status', 1)->limit(12)->latest()->get();
        $categories = new Collection;
        return $categories;
    }
}
