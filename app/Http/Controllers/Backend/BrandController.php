...
class BrandController extends Controller
{
    ...
    public function BrandStore(Request $request){
        $this->validate($request,[
            ...
            'brand_image' => 'required|mimes:jpeg,png,jpg,webp',
        ]);

        if($request->hasfile('brand_image')){
            $image = $request->file('brand_image');
            $name_gen = hexdec(uniqid()).'.'.$image->getClientOriginalExtension();
            Image::make($image)->resize(167,65)->save('upload/brand/'.$name_gen);
    ...
    public function BrandUpdate(Request $request, $id){
        $this->validate($request,[
            'name_en' => 'required',
            'brand_image' => 'nullable|mimes:jpeg,png,jpg,webp',
        ]);
        $brand = Brand::find($id);
        //Brand Photo Update
        if($request->hasfile('brand_image')){
          ...
          /*  $brand_save = time().$brand_image->getClientOriginalName();
            ...
            Image::make($brand_image)->resize(167,65)->save('upload/brand/'.$name_gen);
