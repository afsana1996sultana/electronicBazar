...
class ProductController extends Controller
{
    ...
    public function ProductView(Request $request){
        ...
        if($request->has('search') && $request->search != null){
            $products = $products->where('name_en', 'like', "%$request->search%")->with('category');
            // dd($products);
        }
        $products = $products->latest()->paginate(450);
    	return view('backend.product.product_view',compact('products','p_count'));
    } // end method

    /*=================== Start ProductAdd Methoed ===================*/
    public function ProductAdd(){
    	$categories = Category::where('parent_id', 0)->where('id', '>', 1)->with('childrenCategories')->orderBy('name_en','asc')->get();
		$brands = Brand::latest()->get();
        $vendors = Vendor::latest()->get();
        $suppliers = Supplier::latest()->get();
        $attributes = Attribute::latest()->get();
        $warranties = Warranty::latest()->get();
        $pc_buildings = PCBuilding::orderBy('priority','asc')->get();

    	return view('backend.product.product_add',compact('categories','brands','vendors','suppliers','attributes','warranties','pc_buildings'));
    ...
    public function StoreProduct(Request $request){
        ...
        if($request->warranty_id == null || $request->warranty_id==""){
            $request->warranty_id = 0;
        }
        ...
        $product = Product::create([
            ...
            'pc_building_id'        => $request->pc_building_id,
        ...
        if ($request->has('specs')) {
            ...
            foreach ($request->specs as $key => $spec) {
                $spec_value_data = explode("_", $spec);

                // Check if the array has at least two elements before accessing them
                if (count($spec_value_data) >= 2) {
                    ProductSpec::create([
                        'product_id'    => $product->id,
                        'spec_id'       => $request->spec_ids[$i],
                        'spec'          => $request->spec_names[$i],
                        'spec_value_id' => $spec_value_data[1],
                        'spec_value'    => $spec_value_data[0],
                    ]);
                }
                $i++;
            }
    ...
    public function EditProduct($id){
        ...
        $specs_arr = json_decode($product->category->specs);
        ...
        if ($specs_arr) {
            foreach ($specs_arr as $spec_id) {
                ...
                if ($spec) {
                    $spec->values = $spec->spec_values;
                    $specs->add($spec);
                }
            }
        }
        
        $product_specs = ProductSpec::where('product_id',$id)->get();
        $warranties = Warranty::latest()->get();
        return view('backend.product.product_edit',compact('categories','vendors','suppliers','brands','attributes','product','multiImgs', 'specs', 'product_specs','warranties','pc_buildings'));
    ...
    public function ProductUpdate(Request $request, $id){
        ...
        if($images == Null){
            ...
            foreach ($images as $img) {
                $make_name = hexdec(uniqid()).'.'.$img->getClientOriginalExtension();
                Image::make($img)->resize(438,438)->save('upload/products/multi-image/'.$make_name);
                $uploadPath = 'upload/products/multi-image/'.$make_name;
    ...
    public function getSpecsByCategory($id){
        $category = Category::find($id);
        $specs_arr = json_decode($category->specs);

        //dd($specs_arr);
        $specs = new Collection;
        if($specs_arr){
            for($i=0; $i<count($specs_arr); $i++){
                $spec = Spec::find($specs_arr[$i]);
                $spec->values = $spec->spec_values;
                $specs->add($spec);
                //dd($spec->spec_values);
            }
