...
class PosController extends Controller
{
    ...
    public function updatePosCartQuantity(Request $request)
    {
        $product_id = $request->input('product_id');
        $stock_id = $request->input('stock_id');
        ...
        if ($cart){
            if ($cart->product->discount_type == 1) {
             ...
             if (isset($stock_id)){
                    if ($prod_attr->qty < $quantity) {
                      return response()->json([
                          'error'=> 'Product stock limited',
                           'quantity' => $prod_attr->qty,
                          ]);
                    }
                }
                if (!isset($stock_id)){
                    if ($quantity > $prod_check->stock_qty) {
                        return response()->json([
                            'error'=> 'Product stock limited',
                             'quantity' => $prod_check->stock_qty,
                            ]);
                    }
                }
    ...
    public function insertPosCartSerial(Request $request)
    {
        //dd($request);
        $product_id = $request->input('product_id');
        $stock_id = $request->input('stock_id');
        ...
        if ($cart) {
            $cart->serial_no = $serial_no;
            $prod_check ->serial_no = $serial_no;
            $cart->save();
            $prod_check->save();
            return response()->json([
                'status' => 'success',
                'message' => "Serial inserted successfully",
                'serial_no' => $serial_no,
            ]);
        } else {
            return response()->json([
    ...
    public function store(Request $request)
    {
        ...
        if(get_setting('otp_system')){
            $sms_template = SmsTemplate::where('name','pos_order_message')->where('status',1)->first();
            //dd($sms_template);
            if($sms_template){
