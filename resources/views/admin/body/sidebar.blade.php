...
<aside class="navbar-aside bg-primary" id="offcanvas_aside">
    ...
    <nav>
        <ul class="menu-aside">
            ...
            <li class="menu-item has-submenu
                {{ ($prefix == 'admin/product') || ($prefix == 'admin/category') || ($route == 'attribute.index') || ($route == 'specifications.index') || ($route == 'pc-building.index') || ($route == 'pc-building.create') || ($route == 'pc-building.edit') || ($prefix == 'admin/brand') ? 'active' : '' }}
            ">
                ...
                <div class="submenu">
                    ...
                    @if(Auth::guard('admin')->user()->role == '1' || in_array('53', json_decode(Auth::guard('admin')->user()->staff->role->permissions)))
                        <a class="{{ ($route == 'specifications.index') ? 'active':'' }}" href="{{ route('specifications.index') }}">Specifications</a>
                    @endif
                    @if(Auth::guard('admin')->user()->role == '1' || in_array('57', json_decode(Auth::guard('admin')->user()->staff->role->permissions)))
                        <a class="{{ ($route == 'warranty.index') ? 'active':'' }}" href="{{ route('warranty.index') }}">Warranty</a>
                    @endif
                    ...
                    @endif
                    @if(Auth::guard('admin')->user()->role == '1' || in_array('9', json_decode(Auth::guard('admin')->user()->staff->role->permissions)))
                        <a class="{{ ($prefix == 'admin/pc-buildings') ? 'active':'' }}" href="{{ route('pc-building.index') }}">PC Building</a>
                    @endif
                    @if(Auth::guard('admin')->user()->role == '1' || in_array('1', json_decode(Auth::guard('admin')->user()->staff->role->permissions)))
            ...
            <li class="menu-item has-submenu {{ ($route == 'all_orders.index') || ($route == 'all_orders.pos_sale_index') ?'active':'' }}">
                ...
                <div class="submenu">
                    @if(Auth::guard('admin')->user()->role == '1' || in_array('17', json_decode(Auth::guard('admin')->user()->staff->role->permissions)))
                        <a class="{{ ($route == 'all_orders.index') ? 'active':'' }}" href="{{ route('all_orders.index') }}" >Online Sell</a>
                    @endif

                    @if (Auth::guard('admin')->user()->role == '1' || in_array('17', json_decode(Auth::guard('admin')->user()->staff->role->permissions)))
                        <a class="{{ $route == 'all_orders.pos_sale_index' ? 'active' : '' }}" href="{{ route('all_orders.pos_sale_index') }}">Pos Sell</a>
                    @endif
                </div>
            </li>
            ...
            @if (Auth::guard('admin')->user()->role == '1')
                <li class="menu-item has-submenu   {{ $route == 'review.index' ? 'active' : '' }} ">
                    <a class="menu-link" href="#">
                        <i class="icon material-icons md-stars"></i>
                        <span class="text">Rating & Review</span>
                    </a>
                    <div class="submenu">
                        <a class="{{ $route == 'review.index' ? 'active' : '' }}"
                            href="{{ route('review.index') }}">Rating & Review List</a>
                    </div>
                </li>
            @endif
            ...
            <ul class="menu-aside">
                ...
                ">
                    ...
                    <div class="submenu">
                        ...
                        <a class="{{ ($route == 'emiContent.index') ? 'active':'' }}" href="{{ route('emiContent.index') }}">Emi Content</a>
                    </div>
                </li>
            </ul>
        @endif
