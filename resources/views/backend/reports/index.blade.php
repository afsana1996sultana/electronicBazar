		                   ...
		                   <div class="card-body">
				                <form action="{{ route('stock_report.index') }}" method="GET">
				                    <div class="form-group row mb-3">
				                        <label class="col-md-4 col-form-label">Sort by Category : {{ $categories->name_en ?? ''}}</label>
				                        <div class="col-md-6">
				                        	<div class="custom_select">
											  ...
											  @foreach (\App\Models\Category::all() as $key => $category)
				                                    <option value="{{ $category->id }}" @if ($category->id==$sort_by) selected @endif >{{ $category->name_en }}</option>
				                                @endforeach
											</select>
				                        	</div>
				                        </div>
				                ...
				                <div class="table-responsive-sm" id="order_print">
				                    <table  class="table table-bordered table-hover mb-0" id="order_print" width="100%">
    				                    <thead>
    				                        <tr>
    				                            <th>Product Name</th>
    						                        ...
    						                        @foreach ($products as $key => $product)
    						                        	@if($product->is_varient)
    						                        		@foreach ($product->stocks as $key => $stock)
    					                        				<tr>
    								                                ...
    								                                <td class="text-center">{{ $stock->pre_qty }}</td>
    						                        	...
    						                        	@else
    							                            <tr>
                                                                ...
                                                                <td class="text-center">{{ $product->purchase_price }}</td>
                                                                <td class="text-center">{{ $product->regular_price }}</td>
    							                                <td class="text-center">{{ $product->previous_stock }}</td>
