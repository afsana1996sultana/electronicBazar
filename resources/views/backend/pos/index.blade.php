...
@section('admin')
    @push('css')
        <style>
            ...
            input.form-control.input_serial_no {
                width: 55% !important;
...
@push('footer-script')
    ...
    <script>
        ...
        $(document).on('change', '.inputQty', function () {
            ...
            var qty = Math.max(parseInt(quantity), 1);
            $(this).val(qty);
            var url = '{{ route('pos.cart.update.quantity') }}';
            //console.log('akash vora tara product id', product_id);
            //console.log('akash vora tara stock id', stock_id);
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.status == 'success') {
                        toastr.success(response.message, 'message');
                        var currentPrice = parseFloat($('#totalPrice').text());
                        var updatePrice = parseInt(response?.quantity) * currentPrice;
                        currentPrice = parseFloat(updatePrice);
        
                        $('.totalPrice').text(currentPrice);
                        getCartData();
                    } else {
                        toastr.error(response.error, 'Error');
                        $('.inputQty').val(response?.quantity);
        ...
        // input serial number
        $(document).on('change', '.inputSerial', function () {
            var product_id = $(this).attr('data-product-id');
            var stock_id = $(this).attr('data-stock-id');
            var serial_no = $(this).val(); // Get the serial number value from the input
            var url = '{{ route('pos.cart.insert.serial') }}';
