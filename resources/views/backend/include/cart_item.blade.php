@forelse($pos_cart_data as $cart)
    ...
    <tr class="cart__product">
        <td style="width: 45%;">
            ...
            <!-- product cart price -->
            <p class="product_cart_price d-flex"> 
                <span class="product_price_amount mr-10">{{ __($price) }}{{ __($setting->site_currency) }}</span>
                <input class="form-control input_serial_no inputSerial" type="text" name="serial_no" value="{{ $cart->serial_no }}" data-product-id="{{ $cart->product_id }}" data-stock-id="{{ $cart->stock_id }}">
            </p>
        </td>
        <td style="width: 30%;" >
            <!-- product quantity -->
            <div class="input-group quantity__btn__change">
                ...
                {{-- <input class="form-control text-center quantity_input quantity__number" value="{{ $cart->quantity }}" min="1"
                    ...
                    <input class="form-control text-center quantity_input quantity__number inputQty" value="{{ $cart->quantity }}" min="1" max="" type="text" name="quantity" data-product-id="{{ $cart->product_id }}" data-stock-id="{{ $cart->stock_id }}">
                <!-- incress -->
