                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <form method="post" action="{{ route('category.update', $category->id) }}" enctype="multipart/form-data">
                                        ...
                                        <div class="custom_select">
                                            <select class="form-control select-active w-100 form-select select-nice" name="parent_id" id="parent_id" data-selected="{{ $category->parent_id }}">
                                                ...
                                                @foreach ($categories as $acategory)
                                                    <option value="{{ $acategory->id }}" @if($acategory->id==$selectedCategory) selected  @endif>{{ $acategory->name_en }}</option>
								...
								<div class="form-group row mb-4">
			                        <label class="col-md-3 col-form-label" for="specs">Specification</label>
			                        <div class="col-md-9">
			                        	<div class="custom_select cit-multi-select">
											<select class="form-control select-active w-100 form-select select-nice" name="specs[]" id="specs" multiple="multiple" data-placeholder="Choose Specification">
                                                @foreach($specs as $spec)
                                                    @if($category->specs && count(json_decode($category->specs)) > 0)
                                                        <option @if(in_array($spec->id, json_decode($category->specs))) selected @endif value="{{ $spec->id }}">{{ $spec->name }}</option>
                                                    @else
                                                        <option value="{{ $spec->id }}">{{ $spec->name }}</option>
                                                    @endif
                                                    {{-- <option value="{{ $spec->id }}" @if($category->spec == $spec->id) selected @endif>{{ $spec->name }}</option> --}}
                                                @endforeach
											</select>
										</div>
			                        </div>
                                ...
                                <div class="mb-4">
                                 ...
                                 <div class="mb-4">
                                    <img id="showImage" class="rounded avatar-lg" src="{{ asset($category->image) }}" alt="No image" width="100px" height="80px;">
                                </div>

                                <div class="mb-4">
                                    <label for="" class="col-form-label" style="font-weight: bold;">Background Image:</label>
                                    <input name="backgroundimg" class="form-control" type="file" id="backgroundimg">
                                </div>
                                 <div class="mb-4">
                                    <img id="backgroundimg" class="rounded avatar-lg" src="{{ asset($category->backgroundimg) }}" alt="No image" width="100px" height="80px;">
                                </div>
